...
const steamapis_key = process.env.STEAMAPIS_KEY || "YEyRd9rkFI2DhXV-1_uLFyfznpA";
 
// Whether to skip fetching backpack/profile data (default: false)
const skip_backpack = process.env.SKIP_BACKPACK || false;
...
const config = {
  ...
  database: process.env.MYSQL_DATABASE || 'skins',
    ...
    for (let i = 0; i < steamApisResponse.length; i++) {
      ...
      setTimeout(async () => {
        try {
          ...
          if (response.status == 200) {
            ...
            response.data.median_avg_prices_365days.forEach((data, index) => {
              ...
              if (
                ...
                const calculatedData = calculateData(prices, amount);
                console.log(calculatedData)
                let icon_url = response.data.image.split("/image/");
                let icon = icon_url[1]; /*SERVERS CAN BE DIFFERENT BUT FULL URL NOT WORKING*/
                ...
                if (icon.length < 10) {
                  ...
                  if (tags.length == 0) {
                    ...
                    assets.forEach((data) => {
                      if (
                        data.value.includes("The") && data.value.includes("Collection")
                      ) {
                    ...
                    let updateQuery = `UPDATE skins SET icon = ?, steam365_prices = ?, steam365_amount = ?, firstDate = ?, lastDate = ?, offers_change = ?, offers_change_percentage = ?, sales = ?, sales_percentage = ? ${sql} WHERE name = ?;`;
                    DB.query(
                      updateQuery,
                      [
                        icon,
                        ...
                        days[0],
                        days[1],
                        JSON.stringify(calculatedData.offers_change),
                        JSON.stringify(calculatedData.offers_change_percentage),
                        JSON.stringify(calculatedData.sales),
                        JSON.stringify(calculatedData.sales_percentage),
                        item.name,
                      ],
          ...
          const missingData = {
            'Souvenir AK-47 | Gold Arabesque (Factory New)': 'The Italy Collection',
            'Souvenir AK-47 | Gold Arabesque (Minimal Wear)': 'The Italy Collection',
            'Souvenir AUG | Condemned (Factory New)': 'The Overpass Collection',
            'Souvenir AWP | Desert Hydra (Factory New)': 'The Canals Collection',
            'Souvenir AWP | Dragon Lore (Battle-Scarred)': 'The Cobblestone Collection',
            'Souvenir AWP | Dragon Lore (Factory New)': 'The Cobblestone Collection',
            'Souvenir AWP | Dragon Lore (Field-Tested)': 'The Cobblestone Collection',
            'Souvenir AWP | Dragon Lore (Minimal Wear)': 'The Cobblestone Collection',
            'Souvenir AWP | Dragon Lore (Well-Worn)': 'The Cobblestone Collection',
            'Souvenir AWP | Pit Viper (Battle-Scarred)': 'The Mirage Collection',
            'Souvenir AWP | Safari Mesh (Factory New)': 'The Dust Collection',
            'Souvenir Desert Eagle | Mudder (Factory New)': 'The Vertigo Collection',
            'Souvenir Dual Berettas | Cobalt Quartz (Well-Worn)': 'The Cache Collection',
            'Souvenir FAMAS | Cyanospatter (Factory New)': 'The Nuke Collection',
            'Souvenir M4A1-S | Imminent Danger (Factory New)': 'The Train Collection',
            'Souvenir M4A1-S | Imminent Danger (Minimal Wear)': 'The Train Collection',
            'Souvenir M4A1-S | Imminent Danger (Well-Worn)': 'The Train Collection',
            'Souvenir M4A1-S | Nitro (Factory New)': 'The Mirage Collection',
            'Souvenir M4A1-S | Welcome to the Jungle (Factory New)': 'The Canals Collection',
            'Souvenir M4A1-S | Welcome to the Jungle (Minimal Wear)': 'The Canals Collection',
            'Souvenir MP9 | Orange Peel (Factory New)': 'The Overpass Collection',
            'Souvenir P2000 | Granite Marbleized (Factory New)': 'The Dust Collection',
            'Souvenir P250 | Nuclear Threat (Factory New)': 'The Nuke Collection',
            'Souvenir Tec-9 | Nuclear Threat (Factory New)': 'The Nuke Collection',
            'Souvenir USP-S | Forest Leaves (Factory New)': 'The Vertigo Collection'
          };
...
function main() {
  ...
  const createTableQuery = `CREATE TABLE IF NOT EXISTS skins (
        ...
        rating_count INT,        
        offers_change JSON,
        offers_change_percentage JSON,
        sales JSON,
        sales_percentage JSON,
        is_stattrak TINYINT(1) DEFAULT 0,
        icon VARCHAR(350),
    ...
    if (error) {
      ...
      DB.query(createCsgoBackpackTableQuery, async function (error, result) {
        if (error) {
          ...
          if (skip_backpack) {
            let csgoBackpackPromises = [];
          } else {
 
            console.log(colors.green+"CSGOBackpack.net request starting.")
            ...
            console.log(responseBody.slice(0, 50));
            ...
            if (response.success == true) {
                ...
                for (var i = 0; i <= data.length - 1; i++) {
                    ...
                    var output = item.name.replace(" (Factory New)", "").replace(" (Minimal Wear)", "").replace(" (Field-Tested)", "").replace(" (Well-Worn)", "").replace(" (Battle-Scarred)", "");
                    if (output.includes("Sticker") || output.includes("Graffiti") || /Distinguished|Exceptional|Superior|Master/.test(item.rarity)) {
                        var skin = output.replace(/&#39/g, "'");
                    } else {
                        var skin = output.split(/[|]+/).pop().trim().replace(/&#39/g, "'");
                    }
                    //console.log(items)
                    // INSERT INFO DATA into skins table
                    const promise = queryPromise(`INSERT INTO skins (
                    name, gun_type, skin, rarity, exterior, icon, weapon_type, steam30_avg, steam30_sold) VALUES (
                    ?, ?, ?, ?, ?, ?, ?, ?, ?
                    ...
                    `, [item.name.replace(/&#39/g, "'"), item.gun_type, skin, item.rarity, item.exterior, item.icon_url, item.type, item?.price?.['24_hours']?.average, item?.price?.['24_hours']?.sold, item?.price?.['7_days']?.average, item?.price?.['7_days']?.sold, item?.price?.['30_days']?.average, item?.price?.['30_days']?.sold, item?.price?.['30_days']?.average, item?.price?.['30_days']?.sold]);
                    ...
                    `, [
                        ...
                        item?.price?.['24_hours']?.average, item?.price?.['24_hours']?.median, item?.price?.['24_hours']?.sold,
                        item?.price?.['24_hours']?.standard_deviation, item?.price?.['24_hours']?.lowest_price, item?.price?.['24_hours']?.highest_price,
                        item?.price?.['7_days']?.average, item?.price?.['7_days']?.median, item?.price?.['7_days']?.sold,
                        item?.price?.['7_days']?.standard_deviation, item?.price?.['7_days']?.lowest_price, item?.price?.['7_days']?.highest_price,
                        item?.price?.['30_days']?.average, item?.price?.['30_days']?.median, item?.price?.['30_days']?.sold,
                        item?.price?.['30_days']?.standard_deviation, item?.price?.['30_days']?.lowest_price, item?.price?.['30_days']?.highest_price,
                        item?.price?.all_time?.average, item?.price?.all_time?.median, item?.price?.all_time?.sold,
                        item?.price?.all_time?.standard_deviation, item?.price?.all_time?.lowest_price, item?.price?.all_time?.highest_price
                    ]);
...
// Calculate the data from steamapis.com 1 year history
function calculateData(steam365_prices, steam365_amount, days = [7, 14, 30]) {
  const currentDayIndex = steam365_prices.length - 1;
  const price_change = {};
  const price_change_percentage = {};
  const offers_change = {};
  const offers_change_percentage = {};
  const sales_change = {};
  const sales_change_percentage = {};

  days.forEach((day) => {
    const targetDayIndex = currentDayIndex - day;

    if (targetDayIndex >= 0) {
      const currentPrice = steam365_prices[currentDayIndex];
      const targetPrice = steam365_prices[targetDayIndex];

      price_change[day] = currentPrice - targetPrice;
      price_change_percentage[day] = (((currentPrice - targetPrice) / targetPrice) * 100).toFixed(2);

      offers_change[day] = steam365_prices[targetDayIndex] - steam365_prices[currentDayIndex];
      offers_change_percentage[day] = ((offers_change[day] / steam365_prices[currentDayIndex]) * 100).toFixed(2);

      let salesChange = steam365_amount[currentDayIndex] - steam365_amount[targetDayIndex];
      sales_change[day] = salesChange;
      sales_change_percentage[day] = ((salesChange / steam365_amount[targetDayIndex]) * 100).toFixed(2);
    }
  });
